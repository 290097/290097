...
class Starosta : public Osoba {
private:
    int id_pracownika;
public:
    Starosta() : Osoba(), id_pracownika(0) {}

    void ustawID(int id) { id_pracownika = id; }

    void drukuj() override {
        Osoba::drukuj();
        cout << " [Starosta, ID: " << id_pracownika << "]";
    }
};
            ...
            case 1: {
                if (aktualnaIloscOsob < maxOsob) {
                    ...
                    do {
                        ...
                        if (nowy->ustawDane(im, naz)) { 
                            nowy->ustawIndeks(ind);
                            tablicaOsob[aktualnaIloscOsob] = nowy;
                            sukces = true;
                        }
                        else {
                            delete nowy; 
                    ...
                    if (tablicaOsob[i]->getNazwisko() == klucz) {
                        ...
                        do {
                            ...
                            if (sukces) {
                                ...
                                Student* s = (Student*)tablicaOsob[i];
                                s->ustawIndeks(nind);
